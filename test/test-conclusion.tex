...
%%
%%   (i)  the rule itself adds no vertical space -- the only space it costs
%%        is the clearance the two optional arguments ask for, 0.2em on each
%%        side, so the item sits exactly 0.4em lower than it otherwise would;
...
The conclusion's baseline here must be 0.4em above where it is in the previous
section: the rule costs nothing itself, but the style asks \verb|\propsetaboveitem|
for 0.2em of clearance on each side of it.  Any other difference is a fault.
...
little low.  The two optional arguments of \verb|\propsetaboveitem| are the space
to leave above and below the material; either may be negative.  Equal values
widen the gap and keep the material centred in it; unequal values shift it.

% \SetPropStyle adds to a style rather than replacing it, so a redefinition of
% conclusion has to say what it is not keeping: re-setting wrapper begin alone
% would leave the body begin in place and draw two rules.  Here the key that is
% actually used is the one being varied, so nothing needs clearing.

\newcommand\showraise[3]{%
  \SetPropStyle{conclusion}{body begin =
    \propsetaboveitem[#1][#2]{\smash{%
      \hspace*{-\labelwidth-\labelsep+\itemindent}%
      \rule{\linewidth+\labelwidth+\labelsep-\itemindent}{0.4pt}}}}%
  \begin{prop}
...
\SetPropStyle{conclusion}{body begin =
  \propsetaboveitem[0.2em][0.2em]{\smash{%
    \hspace*{-\labelwidth-\labelsep+\itemindent}%
    \rule{\linewidth+\labelwidth+\labelsep-\itemindent}{0.4pt}}}}
