...
\section{Alignment=right and center}

Labels of varying width show the difference between left, right, and centered
alignment.  Right-aligned labels are flush against the item text (like standard
\texttt{enumerate}); left-aligned labels start at the left margin; centered
labels are centered in the label box.
...
\begin{prop}
    ...
    \pitem[name={A},   align=center] Centered short label.
    \pitem[name={BB},  align=center] Centered medium label.
    \pitem[name={CCC}, align=center] Centered long label.
\end{prop}
...
\begin{prop}
    ...
    \pitem[name={Short label}, align=nextline, labelindent=0pt]
    Nextline with labelindent=0pt (starts at the left margin, replaces left-nextline).
    \pitem[name={A much longer label that should demonstrate how the nextline option with labelindent=0pt lets labels wrap across multiple lines when they are awkwardly long},
    align=nextline, labelindent=0pt]
    Nextline with labelindent=0pt and a long wrapping label.
    \pitem[name={Short label}, align=flush-nextline]
    Flush-nextline with a short label (starts at the item text margin).
    \pitem[name={A much longer label that should demonstrate how the flush-nextline option lets labels wrap across multiple lines when they are awkwardly long},
...
\DeclareNumberedType{TC}[ref format=(\textbf{#1})]

\begin{prop}
    \pitem[type=TC, name={Identity Theory},ref format={(\textbf{the #1})}]
    Mental states are physical states.
    \label{tc:id}
    \pitem[type=TC]
    A plain numbered TC item.
    \label{tc:num}
    \begin{prop}
        \pitem
        Sub-item a.
    ...
    \end{prop}
    \pitem[test,type=TC]
    An unnumbered TC item with lower case name.
    \label{tc:lower}
    \begin{prop}
        \pitem
        Sub-item a.
...
Ref* (titlecase, no link): \Ref*{tc:id} (expected: (\textbf{The Identity Theory}), no hyperlink).
...
Oref (titlecase): \Oref[$'$]{tc:id} (expected: (\textbf{The Identity Theory$'$})).

Lastref: \lastref{} (expected: (\textbf{test})).

Lastref (titlecase): \Lastref{} (expected: (\textbf{Test})).

Nested ref (titlecase): \Ref{tc:sub} (expected: (\textbf{TC2a}) --- TC is
already uppercase).

More examples: \Ref{tc:lower} (expected: (\textbf{Test})),
\Ref{tc:lowersub} (expected: (\textbf{Testa})).

\section{Label format and name format keys}

\DeclareNumberedType{LF}[ref format=(#1)]

%% label format on type declaration
\DeclareNumberedType{LG}[ref format=(#1), label format=#1:]

\begin{prop}
    \pitem[type=LF, label format=#1:]
    Item with label-format colon, declared per-item. \label{lf1}
    \pitem[type=LF, label format=#1:]
    Second item; confirm colon appears. \label{lf2}
    \pitem[type=LF]
    Third item; no colon (label format reset). \label{lf3}
\end{prop}

Expected: \texttt{LF1:} \quad \texttt{LF2:} \quad \texttt{LF3} (no colon).

\begin{prop}
    \pitem[type=LG]
    Type-level label format. \label{lg1}
    \pitem[type=LG, shorthand=SH, gloss=a gloss]
    Item with shorthand and gloss; colon after all. \label{lg2}
\end{prop}

Expected: \texttt{LG1:} \quad \texttt{LG2~[SH]~(a gloss):} (colon after gloss).

%% name format is a synonym for display format
\DeclareNumberedType{NF}[name format=\textit{#1}, ref format=(#1)]

\begin{prop}
    \pitem[type=NF, gloss={should not be italic}]
    Item with name format (italic). \label{nf1}
\end{prop}

Expected: \textit{NF1} in display; \texttt{(NF1)} for ref: \ref{nf1}.
...
\begin{enumerate}
    \item Normal.  
    \item Now going to set leftmargin.  \labelsep=4cm
    \item Now going to set it way back \leftmargin=1cm
    \item Did this do anything I wonder?
\end{enumerate}
