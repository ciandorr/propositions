...
\begin{prop}
  \pitem[type=thesis, Physicalism] Everything is physical. \label{phys}
  \pitem[type=thesis, Dualism] Mind and body are distinct. \label{dual}
\end{prop}
...
\begin{prop}
    \pitem[type=thesis, name={Problems with \ref{myP3}}]
...
\begin{prop}
  ...
  \pitem[type=thesis, Thesis] A long-format item. \label{thesis}
\end{prop}
...
\begin{equation}
  F = ma \ptag[type=thesis, name={Newton's~Law}] \label{eq:newton}
\end{equation}
...
\begingroup
\SetItemType{equation}{display format=\textbf{M#1},
             ref format=M#1}
